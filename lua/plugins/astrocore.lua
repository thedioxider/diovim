...
return {
  ...
  opts = function(_, opts)
    -- Strip `diagnostics.jump.float` from AstroNvim's defaults before
    -- Neovim's config validator sees it (check is `~= nil`, removed in
    -- 0.14). The equivalent `jump.on_jump` is already set upstream.
    if opts.diagnostics and opts.diagnostics.jump then
      opts.diagnostics.jump.float = nil ---@diagnostic disable-line: inject-field
    end
    ---@type AstroCoreOpts
    local user = {
    ...
    return vim.tbl_deep_extend("force", opts, user)
  end,
}
